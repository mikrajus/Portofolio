...
function ProjectCard({ project }: { project: ReturnType<typeof usePortfolioData>["projects"][number] }) {
  ...
  return (
    <Reveal className="w-full">
      <div className="p-6 md:p-8 lg:p-9 rounded-3xl bg-white border border-neutral-200/90 hover:border-amber-500/60 transition-all duration-300 shadow-xl group text-left">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-start">
          {/* Left Column: Details & Flow */}
          <div className={`${project.image ? "lg:col-span-7" : "lg:col-span-12"} space-y-4`}>
            {/* Header: Title & Badges */}
                ...
                <div className="flex flex-wrap items-center gap-1.5 text-xs">
                  {project.architectureFlow.map((step, idx) => (
                    <div key={idx} className="flex items-center gap-1.5">
                      <span className="px-2.5 py-1 rounded-xl bg-white text-neutral-900 border border-neutral-300 font-mono text-[10px] font-bold shadow-xs">
                        {step}
                      </span>
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {project.stack.map((c) => (
                <span
                  key={c}
                  className="px-2.5 py-1 rounded-xl text-[11px] font-mono font-semibold bg-neutral-100 text-neutral-700 border border-neutral-300"
                >
                  {c}
                </span>
              ))}
            ...
            {project.github && (
              <div className="pt-2">
                <a
                  className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold bg-neutral-900 text-white hover:bg-amber-600 transition-all shadow-sm"
                  href={project.github}
                  target="_blank"
          ...
          {imageUrl && (
            <div className="lg:col-span-5 w-full flex justify-center lg:justify-end">
              <div className="relative overflow-hidden rounded-2xl border border-neutral-200 group-hover:border-amber-500/40 transition-colors shadow-md bg-white p-2">
                <img
                  src={imageUrl}
                  alt={project.imageAlt || project.name}
                  width={380}
                  loading="lazy"
                  className="rounded-xl object-contain max-h-56 md:max-h-64 w-auto transform group-hover:scale-102 transition-transform duration-500"
                />
              </div>
            </div>
          )}
  ...
  return (
    <section id="projects" className="section" aria-labelledby="projects-title">
      <div className="container">
        ...
        <Reveal>
          <div className="flex flex-wrap items-center justify-center gap-2.5 mb-10 mt-4">
            <button
        ...
        {/* Projects List with Generous Spacing Between Cards */}
        <div className="flex flex-col gap-10 md:gap-14 my-6">
